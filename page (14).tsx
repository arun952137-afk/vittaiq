  ...
  if (success) return (
    <div style={{ minHeight: '100vh', background: '#080C14', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center', padding: 40 }}>
        ...
        <h2 style={{ color: '#F7FAFC', fontSize: 22, fontWeight: 700, marginBottom: 8 }}>Check your email</h2>
        <p style={{ color: '#718096', fontSize: 14 }}>We sent a confirmation link to <strong style={{ color: '#63B3ED' }}>{email}</strong></p>
        <button onClick={() => router.push('/dashboard')} style={{ marginTop: 24, background: 'linear-gradient(135deg,#63B3ED,#9F7AEA)', border: 'none', color: 'white', borderRadius: 10, padding: '12px 28px', cursor: 'pointer', fontWeight: 700 }}>
          Go to Dashboard →
        </button>
      </div>
    </div>
    ...
    <div style={{ minHeight: '100vh', background: '#080C14', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ width: '100%', maxWidth: 420, background: 'linear-gradient(135deg,#0D1117,#0F1A2E)', border: '1px solid rgba(99,179,237,0.3)', borderRadius: 22, padding: 40 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 800, color: '#F7FAFC', marginBottom: 8 }}>Join VittaIQ</h1>
          <p style={{ color: '#718096', fontSize: 14 }}>Start your AI investing journey</p>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <input style={inp} type="text" placeholder="Full name" value={fullName} onChange={e => setFullName(e.target.value)} required />
          ...
          <p style={{ textAlign: 'center', color: '#718096', fontSize: 13 }}>
            ...
            <span style={{ color: '#63B3ED', cursor: 'pointer', fontWeight: 600 }} onClick={() => router.push('/auth/login')}>Sign In</span>
          </p>
        </div>
      </div>
    </div>
